...
class ValidateService {
  ...
  static Future<Map<String, dynamic>?> getDniData(String dni) async {
    try {
      ...
      final response = await http.get(
        ...
        headers: {
          'Authorization': 'Bearer 88794da72e28b5d99bfbae3f32cfbc4b56328da81388bf11761350dbda7eb94f',
  ...
  static Future<Map<String, dynamic>?> validateCertUnico({
    ...
    try {
      ...
      request.files.add(await http.MultipartFile.fromPath('certUnico', certUnico.path));
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          ...
          return {
            'valido': false,
            'mensaje': errorData['message'] ?? 'Error en la validación (Status: ${response.statusCode})',
            'antecedentes': errorData['antecedentes'] ?? []
          };
        } catch (e) {
          return {
            'valido': false,
            'mensaje': 'Error interno del servidor (Status: ${response.statusCode})',
            'antecedentes': []
          };
        }
      }
